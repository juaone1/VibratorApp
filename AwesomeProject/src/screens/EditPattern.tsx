...
type EditPatterns = {
  navigation: StackNavigationProp<RootStackParamList, 'EditPatterns'>;
};
...
function EditPatterns({ navigation }: EditPatterns) {
      ...
      try {
        ...
        if (savedPatternsString) {
          ...
          if (loadedPattern) {
            ...
            const modifiedPattern = loadedPattern.map(duration => (duration === parseInt(delay) ? 0 : duration));
...
<TouchableOpacity onPress={() => navigation.goBack()} style={styles.backButton}>
  ...
  <View style={styles.header}>
    <Text style={styles.title}>Edit a Pattern</Text>
   </View>
...
<DropDownPicker
            ...
            items={items}
            setOpen={setOpen}
            setValue={setSelectedPatternName}
            setItems={setItems}
            onChangeValue={loadPatternFromStorage}
...
//             }}
//           />
//       <TouchableOpacity style={styles.loadButton} onPress={handleLoadButtonPress}>
...
const styles = StyleSheet.create({
    header: {
      ...
      backButton: {
        ...
        top: 30, // Set the desired distance from the top
        left: 10, // Set the desired distance from the left
